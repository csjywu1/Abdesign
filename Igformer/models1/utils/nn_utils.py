...
# 直接实现scatter函数，避免依赖torch_scatter
def scatter_mean(src, index, dim=-1, out=None, dim_size=None, fill_value=0):
    """
    直接实现的scatter_mean函数，替代torch_scatter.scatter_mean
    """
    if out is None:
        if dim_size is None:
    ...
    if src.shape[0] != index.shape[0]:
        if src.numel() == index.shape[0]:
            src = src.view(index.shape[0], -1)
        else:
            raise ValueError(f"Source tensor first dimension ({src.shape[0]}) must match index tensor first dimension ({index.shape[0]})")
    
    # 使用scatter_add_进行求和
    # 需要确保index和out的维度匹配
    if out.dim() > 1:
        # 对于多维out，需要将index扩展到匹配的维度
        index_expanded = index.unsqueeze(1).expand(-1, src.shape[1])
    ...
    # 避免除零并重塑count以匹配out的维度
    count = torch.clamp(count, min=1)
    count_shape = [count.shape[0]] + [1] * (len(out.shape) - 1)
...
def scatter_sum(src, index, dim=-1, out=None, dim_size=None, fill_value=0):
    """
    直接实现的scatter_sum函数，替代torch_scatter.scatter_sum
    """
    if out is None:
        if dim_size is None:
            dim_size = int(index.max()) + 1
...
class AminoAcidEmbedding(nn.Module):
    ...
    def __init__(self, num_res_type, num_atom_type, num_atom_pos, res_embed_size, atom_embed_size,
        ...
        # Project to 128 dimensions: 16 -> 64, then 64 + 64 = 128
        self.atom_projection = nn.Linear(atom_embed_size, res_embed_size)  # 16 -> 64
    ...
    def forward(self, S, RP, A, AP):
        ...
        # Project atom embedding to res_embed_size and concatenate
        atom_embed_proj = self.atom_projection(atom_embed)  # [N, res_embed_size]
        return torch.cat([res_embed, atom_embed_proj], dim=-1)  # [N, res_embed_size + res_embed_size] = [N, 128]
...
class SeparatedAminoAcidFeature(AminoAcidFeature):
    ...
    def forward(self, X, S, batch_id, k_neighbors, residue_pos=None, smooth_prob=None, smooth_mask=None):
        ...
        # Use the full AminoAcidEmbedding forward method to get 128-dim output
        H = self.aa_embedding(S, residue_pos, atom_type, atom_pos)  # [N, 128]
        
        # atom embedding for edge construction
