...
class E2EDataset(torch.utils.data.Dataset):
    def __init__(self, file_path, save_dir=None, cdr=None, paratope='H3', full_antigen=False, num_entry_per_file=-1, random=False):
        ...
        try:
            with open(metainfo_file, 'r') as fin:
                ...
                # '/home/wujiayang/Igformer-main/all_data/RAbD/train_fixed_processed/part_0.pkl'
                # 前缀替换成 /home/wujiayang/tkde_igformer/all_data/RAbD/train_fixed_processed/part_0.pkl
                # 将前缀 /home/wujiayang/Igformer-main 替换为相对路径
                self.file_names = [file_name.replace('/home/wujiayang/Igformer-main', '.') for file_name in metainfo['file_names']]
    ...
    def __getitem__(self, idx):
        ...
        try:
            cdr_range = item.get_cdr_pos('H3')
            offset = len(ag_data['S']) + 1  # H3在heavy chain中
